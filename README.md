...
- 🔧 **Built-in tools**: Calculator, web search, and custom tools
...
To deploy the agent to AWS Bedrock AgentCore:
...
This will:
1. Initialize the Bedrock AgentCore application
2. Register your agent as a production endpoint
3. Handle incoming requests with enterprise features (auth, memory, observability)

## Custom Tools

The agent includes several custom tools in `src/tools/custom_tools.py`:
...
Then add it to the tools list in `src/tools/custom_tools.py`:
...
| **strands-agents-tools** | 0.2.12 | Pre-built tools (calculator, web_search, etc.) |
...
- **BEDROCK_MODEL_ID**: Bedrock model to use (default: anthropic.claude-4-sonnet-v2:0)
