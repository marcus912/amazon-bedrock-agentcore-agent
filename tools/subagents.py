...
def github_agent(task: str, context: str) -> str:
    ...
    try:
        ...
        logger.info(f"[github_agent] Config GITHUB_AGENT_MODEL_ID: {repr(config.GITHUB_AGENT_MODEL_ID)}")

        # Load GitHub agent system prompt
        system_prompt = load_system_prompt_by_name("github_agent")
        logger.info(f"[github_agent] System prompt loaded, length: {len(system_prompt) if system_prompt else 0}")
        ...
        with github_mcp_client:
            ...
            logger.info("[github_agent] Agent created successfully")

            # Construct the query for the sub-agent
            query = f"Task: {task}\n\nContext:\n{context}"

            # Execute the sub-agent
            logger.info("[github_agent] Invoking sub-agent...")
...
def email_agent(task: str, recipients: str, context: str) -> str:
    ...
    try:
        ...
        logger.info(f"[email_agent] Config EMAIL_AGENT_MODEL_ID: {repr(config.EMAIL_AGENT_MODEL_ID)}")

        # Load Email agent system prompt
        system_prompt = load_system_prompt_by_name("email_agent")
        logger.info(f"[email_agent] System prompt loaded, length: {len(system_prompt) if system_prompt else 0}")

        # Use configured model or None for Strands default (Sonnet 4)
        model_id = config.EMAIL_AGENT_MODEL_ID or None
        logger.info(f"[email_agent] Creating Agent with model_id: {repr(model_id)}")
        ...
        logger.info("[email_agent] Agent created successfully")

        # Construct the query for the sub-agent
        query = f"Task: {task}\n\nRecipients: {recipients}\n\nContext:\n{context}"

        # Execute the sub-agent (will parse details and call send_email_ses)
        logger.info("[email_agent] Invoking sub-agent...")
